...
class RolesInfo(BaseModel):
    ...
    RolesInfo
    """ # noqa: E501
    email: Optional[EmailRoleInfo] = None
    backup: Optional[BackupRoleInfo] = None
    database: Optional[DatabaseRoleInfo] = None
    application: Optional[ApplicationRoleInfo] = None
    dns: Optional[DnsRoleInfo] = None
    webserver_kind: Optional[WebserverKind] = Field(default=None, alias="webserverKind")
    __properties: ClassVar[List[str]] = ["email", "backup", "database", "application", "dns", "webserverKind"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "email": EmailRoleInfo.from_dict(obj["email"]) if obj.get("email") is not None else None,
            "backup": BackupRoleInfo.from_dict(obj["backup"]) if obj.get("backup") is not None else None,
            "database": DatabaseRoleInfo.from_dict(obj["database"]) if obj.get("database") is not None else None,
            "application": ApplicationRoleInfo.from_dict(obj["application"]) if obj.get("application") is not None else None,
            "dns": DnsRoleInfo.from_dict(obj["dns"]) if obj.get("dns") is not None else None,
            "webserverKind": obj.get("webserverKind")
        })
        return _obj
